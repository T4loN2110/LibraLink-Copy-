...
var auth=require('../src/utils/auth-helper');
const fs = require("fs");
...
const requestBorrow=require('../src/models/requestBorrowModel');
...
router.get('/all_invoices_management',async (req, res) => {
  ...
  if(type=="All")
  {
    ...
    query1 = {loai_hd:new RegExp(type)}
  }
  if (type2=="All")
  {
    query2 = {}
  ...
  var invoice = await invoiceModel.find((query1,query2)).skip((perPage * page) - perPage).limit(perPage)
  const count = await invoiceModel.countDocuments({},(query1,query2))
...
router.get('/delete_invoice',async (req, res) => {
  ...
  res.redirect("/all_invoices_management")
